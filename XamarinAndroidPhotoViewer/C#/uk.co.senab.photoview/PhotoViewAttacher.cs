...
namespace Blurate
{
	public class PhotoViewAttacher:Java.Lang.Object,IPhotoView,Android.Views.View.IOnTouchListener,IOnGestureListener,Android.Views.ViewTreeObserver.IOnGlobalLayoutListener 
	{
		...
		int ZOOM_DURATION = IPhotoViewConstants.DEFAULT_ZOOM_DURATION;
		...
		private float mMinScale = IPhotoViewConstants.DEFAULT_MIN_SCALE;
		private float mMidScale = IPhotoViewConstants.DEFAULT_MID_SCALE;
		private float mMaxScale = IPhotoViewConstants.DEFAULT_MAX_SCALE;
		...
		{
			return (float)Math.Sqrt((float) Math.Pow(GetValue(mSuppMatrix, Matrix.MscaleX), 2) + (float) Math.Pow(GetValue(mSuppMatrix, Matrix.MskewY), 2));
		}
		...
		{
			if (milliseconds < 0)
				milliseconds = IPhotoViewConstants.DEFAULT_ZOOM_DURATION;
